...
<div class="flex">
            <div id="Main" class="xl:rounded-r transform  xl:translate-x-0  ease-in-out transition duration-1000 flex justify-between items-start h-screen  w-1/5 bg-green-900 flex-col text-gray-100 sticky top-0">
                ...
                <?php
                    ...
                    if (($dompetRow) && ($bulanRow = pg_fetch_assoc($resultbulan))) {
                        ...
                        echo '<a href="saya_Rencana.php?id_dompet=' . $idDompet . '&bulan=' . $bulan_final . '"class="flex jusitfy-start items-center w-full py-3 pl-12 space-x-8 focus:outline-none text-gray-200 rounded-xl bg-green-950 duration-200">
            ...
            <div class="text-xl mx-auto w-3/5">
                ...
                <div class="bg-green-50 w-72 rounded-2xl p-3 shadow-lg">
                    <div>Filter:</div>
                <label for="id_dompet">Dompet: </label>
                <select class="bg-green-50 border-[1px] rounded-md border-green-800" name="id_dompet" id="id_dompet" value="<?php echo $_GET['id_dompet']; ?>" required>
                <?php
                ...
                <input class="bg-green-50 border-[1px] rounded-md border-green-800" type="month" min="01" max="12" name="bulan" value="<?php echo $_GET['bulan']; ?>" required>
                </div>
                </div>
                <div>
                <button class="bg-green-50 hover:bg-green-800 p-1 px-2 rounded-2xl text-green-800 hover:text-green-50 shadow-lg border-2 border-green-800" name="submit" type="submit">Show</button>
                </div>
                </form>
    </div></p>
<div class="mb-10 mt-2 rounded-xl p-2 px-5 bg-green-50 shadow-lg">
    <p><div>
...
<div id="tabel-container">
        <table id="tabel-pengeluaran" class='w-full table-auto text-left text-md'>
            ...
            <?php
                if($result == TRUE){
                    while ($row_pengeluaran = pg_fetch_assoc($pengeluaran)) {
                        ...
                        echo "<tr class='bg-green-50 border-b'>";
                            echo "<td class='px-4 py-3'>".$row_pengeluaran["nama_tipe"]."</td>";
                        ...
                        if ($row_anggaran = pg_fetch_assoc($result_anggaran)) {
                            ...
                            echo "<div class='h-8 rounded-full w-8 bg-green-800 text-center shadow-lg text-2xl'>
                        <a href='form_Tambah_Anggaran.php?id_tipe=" . $row_pengeluaran['id_tipe'] . "&id_dompet=" . $idDompet . "&bulan=" . $bulan . "'>+</a>
                        </div>";
                        }
                        ...
                        if ($cek_anggaran){
                            echo "<td class='px-4 py-3'>";
                                echo "<a href='proses_Hapus_Anggaran.php?id_tipe=" . $row_pengeluaran['id_tipe'] . "&id_dompet=" . $idDompet . "&bulan=" . $bulan . "' class='px-1 pl-3 h-12 rounded-md border-green-700 border-2  bg-green-50 hover:bg-green-700'>Hapus</a>";
                            echo "<a class='text-green-50'>|</a>";
                                echo "<a href='form_Edit_Anggaran.php?id_tipe=" . $row_pengeluaran['id_tipe'] . "&id_dompet=" . $idDompet . "&bulan=" . $bulan . "' class='px-1 pl-3 h-12 rounded-md border-green-700 border-2  bg-green-50 hover:bg-green-700'>Edit</a>";
                            echo "</td>";
                        } else {
                            echo "<td class='px-4 py-3'>";
                                echo "<a href='form_Tambah_Anggaran.php?id_tipe=" . $row_pengeluaran['id_tipe'] . "&id_dompet=" . $idDompet . "&bulan=" . $bulan . "' class='px-1 pl-3 h-12 rounded-md border-green-700 border-2  bg-green-50 hover:bg-green-700'>Hapus</a>";
                            echo "<a class='text-green-50'>|</a>";
                                echo "<a href='form_Tambah_Anggaran.php?id_tipe=" . $row_pengeluaran['id_tipe'] . "&id_dompet=" . $idDompet . "&bulan=" . $bulan . "' class='px-1 pl-3 h-12 rounded-md border-green-700 border-2  bg-green-50 hover:bg-green-700'>Edit</a>";
                            echo "</td>"; 
                        }
